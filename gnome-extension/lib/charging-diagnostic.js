...
function activePdoWatts(pdPort) {
    const active = pdPort.sourceCapabilities.find(p => p.isActive);
    return active ? Math.floor(active.powerMW / 1000) : 0;
}
...
export function evaluate(pdPort, cable) {
    ...
    // the active PDO via /sys (see TODO in power-delivery.js#parsePdo), this is
    // always 0 and the DeviceLimit branch below stays dormant.
    const activeW = activePdoWatts(pdPort);
